...
public class LogAop {
    ...
    @Before("execution(* com.example.userservice..*(..)) && !execution(* com.example.userservice.exception..*(..))")
    public void before(JoinPoint joinPoint){
        log.info("[log] {} -> {} 실행", joinPoint.getSignature().getDeclaringType().getSimpleName() ,joinPoint.getSignature().getName());
    }

    @After("execution(* com.example.userservice..*(..)) && !execution(* com.example.userservice.exception..*(..))")
