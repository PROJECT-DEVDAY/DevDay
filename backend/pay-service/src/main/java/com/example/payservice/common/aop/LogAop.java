package com.example.payservice.common.aop;
...
public class LogAop {
    ...
    @Before("execution(* com.example.payservice..*(..)) && !execution(* com.example.payservice.common..*(..))")
    public void before(JoinPoint joinPoint){
        log.info("[log] {} -> {} 실행", joinPoint.getSignature().getDeclaringType().getSimpleName() ,joinPoint.getSignature().getName());
    }

    @After("execution(* com.example.payservice..*(..)) && !execution(* com.example.payservice.common..*(..))")
